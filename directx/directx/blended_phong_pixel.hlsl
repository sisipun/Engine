...
float4 main(float3 worldPos : Position, float3 norm : Normal, float3 color : Color) : SV_Target
{
    ...
    return float4(saturate((ambientLight + diffuseLight + specularLight) * color), 1.0f);
}